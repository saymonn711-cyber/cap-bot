...
def get_streams_for_buyer(buyer_notion_id: str):
    ...
    while True:
        ...
        for page in data["results"]:
            ...
            if not cap_info:
                continue
            cap_num = cap_info  # передаём dict дальше
            status = ""
            ...
            streams.append({
                "notion_id":  page["id"],
                "ln_id":      ln_id,
                "cap":        cap_num,  # {"type": "total"|"daily", "value": N}
                "cap_raw":    cap_raw,
                "status":     status,
                "notion_url": page["url"],
            })
...
def parse_cap(raw: str):
    ...
    if raw.startswith("D"):
        ...
        return {"type": "daily", "value": int(nums[0])} if nums else None
    nums = re.findall(r"\d+", raw)
    return {"type": "total", "value": int(nums[-1])} if nums else None
...
def get_campaigns_by_group(group_name: str):
    ...
    # Debug: показываем первые 5 названий
    for c in all_camps[:5]:
        log.info(f"  Кампания: {c.get('name', '')}")
    return all_camps
...
def check_caps_for_user(buyer_notion_id: str, keitaro_group: str):
    ...
    for s in streams:
        ...
        if not cids:
            lines.append(f"⚪ <b>{ln_id}</b> — нет в Keitaro (искал по началу названия)")
            continue

        cap_info = s["cap"]
        cap_type  = cap_info["type"]   # "total" или "daily"
        cap_val   = cap_info["value"]
        cap_label = s["cap_raw"]        # например D5 или T20

        # Считаем нужный период
        if cap_type == "daily":
            actual = sum(stats_7d.get(c, {}).get("sales", 0) + stats_7d.get(c, {}).get("deposits", 0) for c in cids)
            period_label = "сегодня"
        else:
            actual = sum(stats_90d.get(c, {}).get("sales", 0) + stats_90d.get(c, {}).get("deposits", 0) for c in cids)
            period_label = "тотал"
        ...
        if pct >= 1.0:
            ...
            lines.append(f"🔴 {link} [{cap_label}] — <b>ПЕРЕЛИВ на {overflow} FD</b> ({actual}/{cap_val} {period_label}) — нужен стоп!")
            actions.append({"ln_id": ln_id, "notion_id": s["notion_id"], "action": "stop"})
        elif cost_7d == 0:
            lines.append(f"⚠️ {link} [{cap_label}] — <b>не льётся {NO_TRAFFIC_DAYS} дней</b> → поставить холд?")
            actions.append({"ln_id": ln_id, "notion_id": s["notion_id"], "action": "hold"})
        elif remaining <= 0:
            lines.append(f"🟡 {link} [{cap_label}] — капа закрыта ({actual}/{cap_val} {period_label})")
        else:
            lines.append(f"✅ {link} [{cap_label}] — ещё <b>{remaining} FD</b> ({actual}/{cap_val} {period_label})")
...
def handle_message(msg: dict, users: dict, states: dict):
    ...
    if text == "/debug":
        tg_send(chat_id, "⏳ Получаю список кампаний из Keitaro...")
        try:
            headers = {"Api-Key": KEITARO_KEY}
            r = requests.get(f"{KEITARO_URL}/admin_api/v1/campaigns", headers=headers, timeout=15)
            r.raise_for_status()
            ...
            msg = f"Всего кампаний: {len(camps)}
