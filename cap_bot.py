...
def get_offers_by_group(group_name):
    ...
    filtered = [o for o in all_offers
                if str(o.get("group_name", "")).lower() == group_name.lower()
                or str(o.get("group", "")).lower() == group_name.lower()]
...
def check_caps_for_user(group_name):
    ...
    if not group_offers:
        return (f"❌ Группа офферов '<b>{group_name}</b>' не найдена.\n"
                f"Проверь название — оно должно точно совпадать с группой в Keitaro."), []
    ln_to_offer_ids = {}
    ...
    log.info(f"Потоков с матчем: {len(streams)}")
    if not streams:
...
def handle_message(msg, users, states):
    ...
    if text == "/debugoffers":
        tg_send(chat_id, "⏳ Смотрю поля оффера в Keitaro...")
        try:
            headers = {"Api-Key": KEITARO_KEY}
            r = requests.get(f"{KEITARO_URL}/admin_api/v1/offers", headers=headers, timeout=15)
            r.raise_for_status()
            ...
            if offers:
                ...
                group_val = {k: first[k] for k in keys if "group" in k.lower()}
                tg_send(chat_id,
                    f"Всего офферов: {len(offers)}\n\n"
                    f"Поля: {keys}\n\n"
                    f"Group поля первого оффера: {group_val}\n\n"
                    f"Название: {first.get('name', '')}")
    ...
    if state == STATE_ASK_TAG:
        ...
        save_users(users)
        tg_send(chat_id, f"✅ Группа сохранена: <b>{group}</b>", reply_markup=main_keyboard())
        return
...
def get_offers_by_group(group_name):
    ...
    # Фильтруем по group_name в поле group_name оффера
    filtered = [o for o in all_offers
                if o.get("group_name", "").lower() == group_name.lower()
                or o.get("group", {}).get("name", "").lower() == group_name.lower()]

    log.info(f"Офферов в группе '{group_name}': {len(filtered)} из {len(all_offers)}")

    # Если не нашли через group_name — логируем первые 3 для отладки
    if not filtered:
        sample = [{"name": o.get("name",""), "group": o.get("group_name", o.get("group",""))}
                  for o in all_offers[:3]]
        log.warning(f"Группа '{group_name}' не найдена. Примеры офферов: {sample}")
...
def handle_message(msg, users, states):
    ...
    if text == "/debuggroups":
        tg_send(chat_id, "⏳ Проверяю API групп офферов...")
        try:
            headers = {"Api-Key": KEITARO_KEY}
            results = []
            for endpoint in ["groups", "offer_groups"]:
                try:
                    r = requests.get(f"{KEITARO_URL}/admin_api/v1/{endpoint}",
                                   headers=headers, timeout=5)
                    results.append(f"/{endpoint}: {r.status_code} — {str(r.json()[:2])[:200]}")
                except Exception as e:
                    results.append(f"/{endpoint}: ERROR — {e}")
            # Также смотрим поля первого оффера
            r2 = requests.get(f"{KEITARO_URL}/admin_api/v1/offers", headers=headers, timeout=10)
            first = r2.json()[0] if r2.json() else {}
            results.append(f"Поля оффера: {list(first.keys())}")
            tg_send(chat_id, "\n\n".join(results))
        except Exception as e:
            tg_send(chat_id, f"❌ {e}")
        return
