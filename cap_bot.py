...
def get_offers_by_group(group_name):
    ...
    if group_id:
        ...
        return filtered
    log.warning(f"Группа '{group_name}' не найдена — берём все офферы")
    return all_offers
...
# ─── ОТЧЁТ ────────────────────────────────────────────────────────────────────
def check_caps_for_user(tag, keitaro_group):
...
def handle_message(msg, users, states):
    ...
    if text == "🔍 Проверить капы":
        ...
        try:
            report, actions = check_caps_for_user(user["group"], user["group"])
