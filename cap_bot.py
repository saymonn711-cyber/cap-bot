...
def handle_message(msg, users, states):
    ...
    if text == "/debugraw":
        tg_send(chat_id, "⏳ Получаю сырой ответ API для первой записи...")
        try:
            ...
            r2 = requests.post(url2, headers=NOTION_HEADERS, json=payload2, timeout=15)
            r2.raise_for_status()
            data2 = r2.json()
            if data2["results"]:
                page2 = data2["results"][0]
                props2 = page2["properties"]
                resp2 = props2.get("Ответственный", {})
                ln2 = ""
                for key in ["userDefined:ID", "ID", ""]:
                    p2 = props2.get(key, {})
                    if p2.get("type") == "title" and p2.get("title"):
                        ln2 = p2["title"][0]["plain_text"]
                        break
                tg_send(chat_id, f"Поток: {ln2}

Ответственный raw:
{str(resp2)[:500]}")
            else:
                tg_send(chat_id, "Нет результатов")
